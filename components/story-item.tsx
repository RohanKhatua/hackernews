...
export function StoryItem({ id, title, url, score, by, time, descendants, index }: StoryItemProps) {
  const domain = url ? new URL(url).hostname.replace(/^www\./, "") : null
  const formattedTime = formatDistanceToNow(new Date(time * 1000), { addSuffix: true })
